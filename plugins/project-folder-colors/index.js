...
/**
 * Explodex plugin: Project folder colors
 *
 * Color-codes project folders and threads in the sidebar. Hover a row to reveal
 * the color picker on the left. Options live on the Explodex settings page.
 */
(function registerProjectFolderColors(global) {
  const Explodex = global.Explodex;
  if (!Explodex?.plugins?.register) {
    console.warn("[project-folder-colors] Explodex SDK not loaded");
    return;
  }

  const PLUGIN_ID = "project-folder-colors";
