/**
 * Explodex plugin: Usage & Reset Glance (VIEW ONLY)
 *
 * Shows a compact status line above Settings. Click opens a detail popover.
 * Never calls consume/redeem endpoints.
 */
(function registerUsageResetGlance(global) {
  const BC = global.Explodex;
  if (!BC?.plugins?.register) {
    console.warn("[usage-reset-glance] Explodex SDK not loaded");
    return;
  }

  BC.log?.info?.("usage-reset-glance", "script evaluating");

  const SETTINGS_KEY = "explodex-usage-reset-glance";
  ...
  function creditExpiryLabel(credit, formatApi) {
    const unix = creditExpiryUnix(credit);
    if (unix != null) return formatApi.datetimeCountdown(unix);
    const description =
  ...
  function windowContext(win, formatApi) {
    if (!win) {
      return {
        label: "—",
        left: { percent: 0 },
    ...
    return {
      ...
      reset: {
        in: formatApi.countdown(win.resetAt),
        at: formatApi.datetimeCountdown(win.resetAt),
      },
    };
  }

  function buildUsageContext(usage, resets, formatApi) {
    const primary = windowContext(usage?.primary, formatApi);
    const secondary = windowContext(usage?.secondary, formatApi);
    ...
    const resetEntries = resetCredits.map((credit) => ({
      ...
      expires: creditExpiryLabel(credit, formatApi) ?? "—",
    }));
    const resetsCtx = {
  ...
  function formatCompact(usage, resets, template, formatApi) {
    ...
    try {
      return formatApi.template(tpl, buildUsageContext(usage, resets, formatApi), {
        fallback: "—",
      });
    } catch {
      return formatApi.template(DEFAULT_TEMPLATE, buildUsageContext(usage, resets, formatApi), {
        fallback: "—",
      });
    }
  }
  ...
  BC.plugins.register(
    {
      id: "usage-reset-glance",
      name: "Usage and Reset Glance",
      version: "1.3.0",
      viewOnly: true,
    ...
    (api) => {
      ...
      api.migrate([
        {
          ...
          run: ({ renameKey }) =>
            renameKey("explodex-usage-reset-sidebar", SETTINGS_KEY),
        },
      ]);
      ...
      function windowSection(title, win) {
        ...
        block.appendChild(row("Resets", format.datetimeCountdown(win.resetAt), { muted: true }));
        return block;
      }
      ...
      function isNavMounted() {
        return Boolean(
          navButton?.isConnected &&
            document.querySelector('[data-explodex-nav="usage-reset-glance"]')?.isConnected,
        );
      }
