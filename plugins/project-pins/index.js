 ...
 */
(function registerProjectPins(global) {
  const BC = global.Explodex;
  if (!BC?.plugins?.register) {
    console.warn("[project-pins] Explodex SDK not loaded");
    return;
  }

  const PROJECT_PINS_KEY = "explodex-project-pins-pinned-threads";
      ...
      api.migrate([
        {
          ...
          run: ({ renameKey }) =>
            renameKey("explodex-project-pinned-threads", PROJECT_PINS_KEY),
        },
      ]);
