...
import { getInjectorPath, getLauncherPath, getLogDir, getPluginsDir, getSdkPath, getUserPluginsDir, pathExists } from "../paths.mjs";
import { isExplodexOwnedBundle } from "../launcher-bundle.mjs";
...
async function activateCodex() {
  await command("osascript", ["-e", 'tell application "Codex" to activate']);
}

async function confirmRelaunch() {
  const script = 'display dialog "Codex is already running without Explodex. Quit and relaunch it with Explodex?" with title "Explodex" buttons {"Cancel", "Quit and Relaunch with Explodex"} default button 2 cancel button 1';
  try { await execFileAsync("osascript", ["-e", script]); return true; } catch { return false; }
}

async function quitCodexAndWait(timeoutMs = 15_000) {
  ...
  throw new Error("Codex did not quit within 15 seconds. Quit it manually, then reopen Explodex.");
}

async function waitForDebugPort(port, child, timeoutMs = 30_000) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    if (child.exitCode !== null) throw new Error("Codex exited before its debug port opened");
...
export async function launch({ port = Number(process.env.EXPLODEX_DEBUG_PORT ?? DEFAULT_PORT), home = homedir() } = {}) {
  ...
  const codexLog = join(logDir, "codex.log");
  ...
  if (info.state === "plain-codex") {
    if (!(await confirmRelaunch())) { await log("plain-codex relaunch cancelled"); return { state: "cancelled", log: launcherLog }; }
    await splashStatus("Quitting Codex…");
    try { await quitCodexAndWait(); } catch (error) { throw new Error(`${error.message} Logs: ${launcherLog}`); }
    info = { ...info, state: "stopped" };
  }
  if (info.state === "stopped") {
    await splashStatus("Launching Codex…");
    const output = await open(codexLog, "a");
    const env = installedCodexEnvironment();
    const child = spawn(CODEX_BIN, [`--remote-debugging-port=${port}`], { detached: true, stdio: ["ignore", output.fd, output.fd], env });
    child.unref();
    await output.close();
    try { await waitForDebugPort(port, child); } catch (error) { throw new Error(`${error.message}. See ${codexLog} and ${launcherLog}`); }
  }
...
export async function doctor({ port = Number(process.env.EXPLODEX_DEBUG_PORT ?? DEFAULT_PORT), home = homedir() } = {}) {
  const info = await inspectLaunchState(port);
  const userLauncher = getLauncherPath({ home });
  const systemLauncher = getLauncherPath({ home, system: true });
  return {
    platform: "macOS",
    codexInstalled: await pathExists(CODEX_BIN),
    injectorPresent: await pathExists(getInjectorPath()),
    sdkPresent: await pathExists(getSdkPath()),
    userLauncher: { path: userLauncher, installed: await pathExists(userLauncher), owned: await isExplodexOwnedBundle(userLauncher) },
    systemLauncher: { path: systemLauncher, installed: await pathExists(systemLauncher), owned: await isExplodexOwnedBundle(systemLauncher) },
    launcherLog: join(getLogDir(home), "launcher.log"),
    ...info,
  };
}
