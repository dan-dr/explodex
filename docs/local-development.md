...
The shell resolves `explodex --from-app` through a login shell. This means an npm update is used without regenerating a native executable. The CLI starts `/Applications/Codex.app/Contents/MacOS/Codex` with `--remote-debugging-port`, injects package assets, activates Codex, and exits. It does not set `CODEX_ELECTRON_USER_DATA_PATH`, so the normal Codex profile is retained.
