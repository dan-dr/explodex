...
import {
  applyProjectPinOrder as applyProjectPinOrderModel,
  globalPinIdCandidates as globalPinIdCandidatesModel,
  localThreadKey as localThreadKeyModel,
  normalizeConversationId as normalizeConversationIdModel,
  normalizeProjectId as normalizeProjectIdModel,
  normalizeProjectPinsMap as normalizeProjectPinsMapModel,
  pinScopeChoiceActions,
  removeGlobalPinConflicts,
  resolveAssignedProject,
  shouldShowProjectPinIndicator as shouldShowProjectPinIndicatorModel,
  type ProjectOrders,
  type ProjectPinsMap,
} from "./model";
 ...
 */
export const PROJECT_PINS_KEY = "explodex-project-pins-pinned-threads";
export const LEGACY_PROJECT_PINS_KEY = "explodex-project-pinned-threads";
...
export async function setupProjectPins(
  api: PluginApi,
  runtime: ProjectPinsRuntime = globalThis as unknown as ProjectPinsRuntime,
      ...
      let pinStyleElement: HTMLStyleElement | null = null;
      let pinsCache: ProjectPinsMap = {};
      let pinsHydrated = false;
      let pinsHydratePromise: Promise<ProjectPinsMap> | null = null;
      let disposed = false;
      const pendingTimeouts = new Set<number>();
      const pendingDelayResolvers = new Map<number, () => void>();
      ...
      function normalizeConversationId(value) {
        return normalizeConversationIdModel(value);
      }
      ...
      function localThreadKey(conversationId) {
        return localThreadKeyModel(conversationId);
      }

      function normalizeProjectPinsMap(value) {
        return normalizeProjectPinsMapModel(value);
      }
      ...
      function normalizeProjectId(value) {
        return normalizeProjectIdModel(value);
      }
      ...
      async function listGloballyPinnedIds() {
        const res = await bridge.rpc<{ threadIds?: unknown }>(
          "list-pinned-threads",
          { params: {} },
        );
      ...
      function globalPinIdCandidates(threadKey) {
        return globalPinIdCandidatesModel(threadKey);
      }
      ...
      async function setGlobalPinState(threadKey, pinned, nativePinButton = null) {
        ...
        for (const id of globalPinIdCandidates(threadKey)) {
          if (disposed) return;
          const res = await bridge.rpc<{ success?: boolean }>(
            "set-thread-pinned",
            {
      ...
      async function readProjectOrders(): Promise<ProjectOrders> {
        const value = await storage.globalState.get(
          GLOBAL_STATE_KEYS.projectOrders,
        );
        return value && typeof value === "object"
          ? ({ ...value } as ProjectOrders)
          : {};
      }

      async function writeProjectOrders(orders: ProjectOrders) {
        if (disposed) return;
      ...
      function ensurePinScopeStyles() {
        ...
        pinStyleElement = style as HTMLStyleElement;
      ...
      function shouldShowProjectPinIndicatorForRow(row, threadKey) {
        ...
        return shouldShowProjectPinIndicatorModel({
          globallyPinned:
            row.getAttribute("data-app-action-sidebar-thread-pinned") === "true",
          pinnedProjectId: conversationId
            ? normalizeProjectId(projectIdForPinned(conversationId))
            : null,
      ...
      function applySidebarDomReorder(
      ...
      function applyProjectPinOrder(
        orders: ProjectOrders,
        pins: ProjectPinsMap,
      ) {
        const projectThreadIds: Record<string, string[]> = {};
        ...
        return applyProjectPinOrderModel(orders, pins, {
          activityMs,
          projectThreadIds,
        });
      }
      ...
      function triggerNativePin(pinButton) {
        ...
        pinButton.dispatchEvent(
          new global.PointerEvent("pointerdown", {
            ...
            cancelable: true,
            button: 0,
            view: global as unknown as Window,
          }),
        );
        pinButton.click();
        scheduleTimeout(() => {
