...
export default definePlugin({
  async setup(api) {
      ...
      let settings: CommandMenuThreadSettings =
        normalizeCommandMenuThreadSettings(
      ...
      function commandMenuInput(root) {
        return (
          (root?.querySelector("[cmdk-input]") as HTMLInputElement | null) ??
          null
        );
      }
      ...
      function filterThreads(threads, query) {
        return filterThreadCatalog(threads, query, settings);
      }
            ...
            c.sortableList({
              ...
              onReorder: (ids) => {
                settings.sortBy = ids.filter(
                  (id): id is keyof typeof SORT_LABELS => id in SORT_LABELS,
                );
                saveSettings();
              },
            }),
      ...
      function updateInputPlaceholder(root) {
        ...
        if (!input) return;
        if (input.dataset.explodexOriginalPlaceholder === undefined) {
          input.dataset.explodexOriginalPlaceholder =
            input.getAttribute("placeholder") ?? "__absent__";
        }
      ...
      function restoreInputPlaceholder(root) {
        ...
        else if (original !== undefined) input.setAttribute("placeholder", original);
          ...
          if (
            records.every(
              (record) =>
                ...
                record.target.closest(`#${INJECTED_GROUP_ID}`),
            )
          ) {
            return;
          }
