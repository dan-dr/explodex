...
import {
  ...
  type ToggleAutoscrollSettings,
} from "./settings";

/**
 * Explodex plugin: Toggle Autoscroll
 *
 * Adds a per-thread autoscroll toggle above the composer. When disabled, a
 * visible message remains anchored across turn updates.
 */
const global = globalThis;

const STORAGE_KEY = "explodex-toggle-autoscroll";
const CONVERSATION_ID_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

/** @typedef {{ element: HTMLElement, top: number } | { element: null, scrollTop: number }} ScrollAnchor */

export default definePlugin({
  setup(api) {
      const { components: c, inject, log, registerOptions, storage } = api;

      let settings: ToggleAutoscrollSettings = loadSettings();
      ...
      function captureAnchor() {
        ...
        anchor = best
          ? { element: best, top: best.getBoundingClientRect().top }
          : { element: null, scrollTop: scrollElement.scrollTop };
      }
      ...
      function restorePortalStyle() {
        ...
        if (styledPortal.dataset.explodexAutoscrollOwner === api.token) {
          const original =
            styledPortal.dataset.explodexAutoscrollOriginalStyle;
      ...
      function stylePortal() {
        ...
        restorePortalStyle();
        styledPortal = portal;
        if (portal.dataset.explodexAutoscrollOriginalStyle === undefined) {
          portal.dataset.explodexAutoscrollOriginalStyle =
            portal.getAttribute("style") ?? "__absent__";
        }
      ...
      function buildToggle() {
        ...
        root.style.cssText =
          "position:relative;flex:0 0 auto;display:none;align-items:center;pointer-events:auto";
        ...
        button.style.cssText +=
          ";min-width:28px;height:28px;border-radius:999px;padding:0 7px;gap:5px;transition:opacity 120ms,background 120ms";
