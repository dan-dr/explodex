...
export default definePlugin({
  setup(api) {
      ...
      let settings: ProjectColorSettings = defaultProjectColorSettings();
...
`.trim();
       ...
       */
      function applyColorToRow(
        row,
        color,
        kind,
        options: { groupPos?: "first" | "middle" | "last" | "only" } = {},
      ) {
        if (!row) return;
        if (!color) {
      ...
      function renderOptionsPanel(container, { refresh }) {
        ...
        for (const [label, value] of [
          ...
          ["Threads", "threads"],
          ["Both", "both"],
        ] satisfies Array<[string, ColorTarget]>) {
